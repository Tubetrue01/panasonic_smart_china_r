...
class PanasonicConfigFlow(config_entries.ConfigFlow, domain=DOMAIN):
    ...
    async def _create_fresh_air_entry(self):
        return await self._async_create_device_entry(DEVICE_KIND_FRESH_AIR, "Panasonic Fresh Air")

    async def _create_fridge_entry(self):
        return await self._async_create_device_entry(DEVICE_KIND_FRIDGE, "Panasonic Fridge")

    async def _async_create_device_entry(self, device_kind: str, device_name: str | None):
        token = generate_device_token(self._selected_dev_id) or ""
        dev_info = self._devices.get(self._selected_dev_id) or {}
        dev_name = dev_info.get("deviceName", device_name)
